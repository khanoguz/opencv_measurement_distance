...
for c in a_hat:
	...
	cY = np.average(kutu[:, 1])#daha sonra numpy modülündeki average fonsksiyonu ile kutularımızın kordinatlarının ortalamasını alarak kutuların merkezini tesbit ediyoruz.
    ...
    if refObj is None:
	    ...
	    D = dist.euclidean((tlblX, tlblY), (trbrX, trbrY)) # SQRT((trbrx-tlblx)^2+(tbry-tlbly)^2) euclidean fonsksiyonu ile öklid uzunluklarını buluyoruz.
        refObj = (kutu, (cX, cY), D / args["kalinlik"]) # burada refObj'miz 3 elemanli bir demet döndürüyor. ilki kutu, ikinisi refObj'nin merkezi(agırlık merkezi) ve son olarak uzunluklarını
